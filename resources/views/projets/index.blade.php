<x-app-layout>
    <div class="flex justify-between items-center mb-6">
        ...
        <a href="{{ route('projets.create') }}" class="bg-blue-600 hover:bg-blue-700 text-white font-medium py-2 px-4 rounded-lg transition-colors duration-200">
                    ...
                    @forelse($projets as $projet)
                        <tr class="hover:bg-gray-50">
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="text-sm font-medium text-gray-900">{{ $projet->nom_projet }}</div>
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                                <div>Du {{ $projet->date_debut->format('d/m/Y') }}</div>
                                <div>Au {{ $projet->date_fin->format('d/m/Y') }}</div>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                @php
                                    $statusColors = [
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                                {{ $projet->evaluations->count() }} évaluation(s)
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                <div class="flex justify-end space-x-2">
                                    <a href="{{ route('projets.show', $projet) }}" class="text-blue-600 hover:text-blue-900">
                                    ...
                                    <form action="{{ route('projets.destroy', $projet) }}" method="POST" class="inline" onsubmit="return confirm('Êtes-vous sûr de vouloir supprimer ce projet ?')">
                                        @csrf
                                        @method('DELETE')
